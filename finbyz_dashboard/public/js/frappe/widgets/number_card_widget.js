...
export default class NumberCardWidget extends Widget {
	...
	create_number_card() {
		this.set_doc_args();
		return frappe.xcall(
			'finbyz_dashboard.finbyz_dashboard.doctype.number_card.number_card.create_number_card',
			{
				'args': this.card_doc
			}
		).then(doc => {
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			'Document Type': {
				method: 'finbyz_dashboard.finbyz_dashboard.doctype.number_card.number_card.get_result',
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_percentage_stats() {
		return frappe.xcall('finbyz_dashboard.finbyz_dashboard.doctype.number_card.number_card.get_percentage_difference', {
			doc: this.card_doc,
			filters: this.filters,
			result: this.number
		}).then(res => {
