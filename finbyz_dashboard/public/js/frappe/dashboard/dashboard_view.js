		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							'query': 'finbyz_dashboard.finbyz_dashboard.dashboard_overrides.dashboard_chart.get_charts_for_user',
							filters: {
								document_type: this.doctype,
							}
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == 'New Chart') {
					...
					chart.filters_json = '[]';
					frappe.xcall('finbyz_dashboard.finbyz_dashboard.dashboard_overrides.dashboard_chart.create_dashboard_chart', {'args': chart}).then((doc)=> {
